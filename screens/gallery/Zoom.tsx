...
import { ZoomListContext } from "./ZoomListContext";
...
const Zoom = (props: Props) => {
            ...
            .onUpdate((e) => {
                if (isPinching.value || !isZoomed.value) {
                    ...
                    if (nextTranslateX > maxTranslateX) {
                        ...
                        translationX.value = withTiming(minTranslateX);
                    } else {
                        translationX.value = nextTranslateX;
                    }

                    if (nextTranslateY > maxTranslateY) {
                        translationY.value = withTiming(maxTranslateY);
                    } else if (nextTranslateY < minTranslateY) {
                        translationY.value = withTiming(minTranslateY);
                    } else {
        ...
        const doubleTap = Gesture.Tap()
            .onStart((e) => {
                ...
                if (scale.value !== 1) {
                    ...
                    const doubleTapScale = maximumZoomScale / 2;

                    // translate the image to the focal point and zoom
                    scale.value = withTiming(doubleTapScale);
                    translationX.value = withTiming(
                        -1 * (doubleTapScale * (e.x - viewWidth.value / 2))
                    );
                    translationY.value = withTiming(
                        -1 * (doubleTapScale * (e.y - viewHeight.value / 2))
                    );
                }
            })
            .numberOfTaps(2);
    ...
    const memoizedOnLayout = useCallback(
        ...
        [viewHeight.value, viewWidth.value, onLayout]
    );

    return (
        <GestureDetector gesture={gesture}>
            <Animated.View {...props} style={memoizedStyle} onLayout={memoizedOnLayout} />
        </GestureDetector>
    );
};
